...
async function parsepage(snap) {
	return new Promise((resolve, reject) => {
		...
		*/
		...
		snap.page = '🤡';
		resolve(snap);
		if (snap.page === false) reject(snap);
	});
}
/*
TODO: REMOVE ASYN HERE AND USE PROMISE AWAIT ALL IN BODY OF FUNC??
*/
async function addtodb(snap) {
	return new Promise(async (resolve, reject) => {
		...
		const runres = await db.run(stmt);
		resolve(snap);
	});
}
...
async function getpids(pids) {
	return new Promise(async (resolve, reject) => {
		...
		const select_res = await db.all(select_stmt);
